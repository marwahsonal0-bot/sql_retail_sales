...
1. **Write a SQL query to find the total revenue generated by the business.**:
```sql
SELECT SUM(total_sale) AS total_revenue
FROM retail_sales;
```

2. **Write a SQL query to find which product categories generate the highest revenue.**:
```sql
SELECT category,
...
3. **Write a SQL query to find total profit generated by the company.**:
```sql
...
4. **Write a SQL query to find the top 5 highest spending customers.**:
```sql
SELECT customer_id,
...
6. **Write a SQL query to retrieve all transactions where the category is 'Electronics' and the quantity sold is more than 2 in the month of Nov-2022.**:
```sql
SELECT *
FROM retail_sales
WHERE 
...
15. **Write a SQL query to find how does revenue vary month-wise.**:
```sql
...
16. **Write a SQL query to find difference in revenue contribution between male and female customers.**:
```sql
SELECT gender,
       SUM(total_sale) AS revenue
FROM retail_sales
...
17. **Write a SQL query to find which age group generates the highest revenue.**:
```sql
SELECT age,
       SUM(total_sale) AS revenue
FROM retail_sales
...
18. **Write a SQL query to find which category has the highest total quantity sold.**:
```sql
SELECT category,
...
19. **Write a SQL query to find what is the average spending per transaction.**:
```sql
SELECT ROUND(AVG(total_sale)) AS avg_transaction_value
FROM retail_sales
```
20. **Write a SQL query to find transactions where cost exceeded revenue.**:
```sql
SELECT *
FROM retail_sales
...
2. **Set Up the Database**: Run the SQL scripts to create and populate the database.
